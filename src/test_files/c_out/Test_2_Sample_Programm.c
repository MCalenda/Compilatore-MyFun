...
float sommac(int a, float b, char **size) {float result;result = a + b + c;if(result > 100) {char *valore = "grande";*size = valore;}else {char *valore = "piccola";*size = valore;} return result;}void stampa(char *messaggio) {int i = 1;while(i <= 4) {int incremento = 1;printf("%s", "");printf("\r\n"); i = i + incremento;}printf("%s", messaggio);printf("\r\n"); }

// Funzione Main
int main() {int a = 1;float b = 2.2;char *taglia = malloc(512 * sizeof(char));char *ans = "no";float risultato = sommac(a, b, &taglia);printf("%s", "vuoi continuare? (si/no)");printf("\t"); scanf("%s", ans);while(strcmp(ans, "si") {printf("inserisci un intero:");scanf("%d", &a);printf("inserisci un reale:");scanf("%f", &b);risultato = sommac(a, b, &taglia);stampa(concatString( concatString( concatInt( concatString( concatReal( concatString( concatInt( "la somma di ", a), " e "), b), " incrementata di "), c), " è "), taglia));stampa(concatReal( " ed è pari a ", risultato));printf("vuoi continuare? (si/no):\t");scanf("%s", ans);}printf("%s", "");printf("\r\n"); printf("%s", "ciao");return 0; }